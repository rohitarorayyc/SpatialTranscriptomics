...
enrichIM <- enrichList2IM(er_canonical, valueColname = "zScore", 
...
keep <- rownames(binarydf[binarydf$total>9,])
...
hm <- call_fn_ellipsis(ComplexHeatmap::Heatmap, matrix = use_matrix, 
                       ...
                       row_dend_reorder = TRUE, border = TRUE, 
                       row_names_gp = grid::gpar(fontsize = row_fontsize), 
                       row_names_max_width = grid::unit(20, "cm"), column_names_gp = grid::gpar(fontsize = 16), 
                       column_names_max_height = grid::unit(12, "cm"), 
                       cluster_columns = FALSE, row_dend_width = grid::unit(2, "cm"), 
                       column_title = NULL, heatmap_legend_param = list(border = "black", legend_height = grid::unit(8, "cm")),
                       bottom_annotation = ha)

png("IPA_Heatmap_Canonical.png", units = "in", res=300, width=18, height=14)
draw(hm)
dev.off()
